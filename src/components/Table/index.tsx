...
import {TableStyled, Tbody, TableRow} from "./styled";

interface IProps {
  dataUser: IUser[];
  handleSort: () => void;
  handleDeleteUser: (T: string) => void;
  isFilesColors: boolean;
}

const Table: React.FC<IProps> = ({dataUser, handleSort, handleDeleteUser, isFilesColors}) => {
  if (dataUser?.length === 0) return <DefaultContent />;
  ...
  return (
    <TableStyled>
      ...
      <Tbody>
        {dataUser?.map((user: IUser, index: number) => {
          return (
            <TableRow
              key={user.email}
              file={isFilesColors ? (index % 2 === 0).toString() : "false"}
            >
              <td>
              ...
              <td>
                <button
                  className="tooltip"
                  data-testid={`delete-button-${user.email}`}
                ...
                >
                  ...
                  >
                    <path
                      ...
                      fill={isFilesColors ? "#f1f1f1" : "#dbe6e4"}
                      fillRule="evenodd"
                    />
                  </svg>
